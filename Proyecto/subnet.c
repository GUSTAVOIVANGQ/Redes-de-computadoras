...
int ipVerify(char* ipAddress, unsigned char* octetArray) {
	// La funcion verifica que se ha ingresado una IP válida, y luego actualiza el array de octetos con los octetos validados.
	...
	while (token != NULL) {
		...
		// Recorre cada caracter y verifica si es un digito
		// Si no lo es, interrumpe el ciclo. Usamos j para ver si se itera la cantidad correcta de veces
...
// Imprime información sobre la subred dada.
	...
	// Resta la direccion de red de la dirección de broadcast y resta uno del resultado para el total de hosts
	printf("Total hosts: %d\n", ((netAddress | ~netMask) - netAddress) - 1);

	// Muestra la primera direccion de host sumando a cada uno de nuestros octetos desempaquetados
	printf("Primera direccion de host: %d.%d.%d.%d\n", ((netAddress + 1) >> 24) & 0xFF, ((netAddress + 1) >> 16) & 0xFF,
						    ((netAddress + 1) >> 8) & 0xFF, (netAddress + 1) & 0xFF);
	
	//Realiza un OR bit a bit con la direccion int y la máscara negada para obtener la direccion de broadcast en la variable
	netAddress = netAddress | ~netMask;

	// Reste de la direccion de broadcast para obtener la dirección del host final
	printf("Ultima direccion de host: %d.%d.%d.%d\n", ((netAddress - 1) >> 24) & 0xFF, ((netAddress - 1) >> 16) & 0xFF,
						   ((netAddress - 1) >> 8) & 0xFF, (netAddress - 1) & 0xFF);
	
	// Desempaqueta y muestra la direccion de broadcast
...
int main() {
	...
	do {
		...
		while (subnetNumber > totalSubnets) {
			subnetBits++;
			totalSubnets = pow(2, subnetBits) ;// (2^n)
		}
